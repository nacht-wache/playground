...
/*
 * Базовая идея - использовать дерево наблюдателей на std::weak_ptr-ах с глубиной = 4.
 * Убийство/разрушение - std::shared_ptr::reset c CustomDeleter
 ...
 * Расчёт среднего за O(1) за счёт кэширования на уровне объекта (Человек, Город и т.д.)
 */
...
namespace detail {
  ...
  class TBase : public IHandle {
    ...
    static std::shared_ptr<Derived> CreateMe(Args &&...args) {
      return CreateMePrivate(std::default_delete<Derived>{}, std::forward<Args>(args)...);
    }
    ...
    void Settle(TSettlement *settlement) requires std::derived_from<TSettlement, IHandle> {
      if (m_observer) {
  ...
  protected:
    explicit TBase(std::string name) : m_name(std::move(name)) {}

  private:
    std::shared_ptr<IObserver> CreateObserver() requires std::derived_from<TCreate, IHandle> {
      struct Obs : IObserver {
        ...
        void OnMove(IHandle & handle, MoveType type) override {
          if (auto locked = obj.lock()) {
            ...
            locked->m_peopleSize += (type == MoveType::out ? -peopleCount: +peopleCount);
          }
        ...
        explicit Obs(std::weak_ptr<TBase> baseHandle, std::weak_ptr<IObserver> observer)
            : obj(std::move(baseHandle)), m_observer(std::move(observer)) {}
